 ...
 *
 * bufhash	- declared in buf.h and defined in machdep.c as an array of bufhd
 * things BUFHSZ	- macro constant defined in buf.h
 *
 */
 ...
 *
 * bfreelist 	- declared in buf.h as an arrary of "buf" structs. Defined in
 * machdep.c bufdata	- delcared in buf.h as an array of chars. Defined in
 * machdep.c buf		- buf is an array of buf structs. See buf.h and
 * machdep.c binshash()	- macro function defined in buf.h brelse()	-
 * function declared in buf.h - arg is a pointer to a buf struct BQUEQUES
 * - macro constant in buf.h - comment - number of free buffer queues
 ...
 * NBUF		- macro constant in machparam.h - comment - number of i/o
 * buffers MAXBSIZE	- macro constant in params.h - no comments
 * NODEV	- macro constant in param.h defined as (dev_t)(-1) - dev_t is an
 * alias for int
 *
 */
 ...
 * NCLIST	- macro constant defined in machparam.h - number of CBSIZE
 * blocks
 *
 */
 ...
 * proc[] - declared as extern in proc.h - process table. Can't find where its
 * defined p_addr - cpp define in proc.h - p_un.p_alive.P_addr u - declared in
 * user.h as a user struct SRUN - cpp define in proc.h - status code - process
 * running SLOAD, SSYS - cpp define in proc.h - flags SLOAD - process in core
 ...
 * rootdev - declared in systm.h as dev_t and defined in unix.map for each dev
 * board pipedev - declared in systm.h as dev_t and defined in unix.map for each
 * dev board conf_service - struct defined in kconfig.h conf_service_init -
 * array of conf_service structs - declared in kconfig.h and defined in ioconf.c
 * for each dev board spl0() - appears to be a macro define in machparam.h for
 * mips_intr_enable() defined in io.h mountfs - function defined in fs.h mount[]
 * - array of mount structs - defined in mount.h mount_updname - function
 * defined in fs.h boottime - timeval struct - defined in kernel.h major(),
 * minor()  - macro functions defined in types.h swapdev - declared in systm.h -
 * defined in swapunix.c for each dev board bdevsw - array of bdevsw structs -
 * declared in conf.h - defined in devsw.c swapmap - array of map things -
 * declared in map.h - defined in machdep.c mfree - function defined in map.h
